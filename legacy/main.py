...
- app/main_api.py for the main API service (user-facing features)
- app/main_content.py for the content generation service

The application has been split into two separate services:
1. Main API Service: Handles user authentication, cart, orders, payments, etc.
2. Content Generation Service: Handles chapter generation, HTML conversion, PDF generation, etc.
"""
...
def create_app() -> FastAPI:
    """Legacy unified application - use main_api.py or main_content.py for new deployments."""
    app = FastAPI(title="Saras Backend (Legacy)", version="0.1.0", redirect_slashes=False)
    app.add_middleware(
        CORSMiddleware,
